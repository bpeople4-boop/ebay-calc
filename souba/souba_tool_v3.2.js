...
const cfgOf=m=>m==='HERMES'?byKey('HERMES'):/^GUCCI /.test(m)?byKey('GUCCI'):(S&&S.br&&S.br[m]?byKey(S.br[m]):BRANDS.find(b=>b.key!=='GUCCI'&&b.pat.test(m))||byKey('TAG'));
...
const H_GROUPS=['ケリー カデナ','ケリー（カデナ以外）','クリッパー 24mm前後 金','クリッパー 24mm前後 シルバー','クリッパー 24mm前後 コンビ','クリッパー 27〜33mm','クリッパー 34mm〜','ラリー','Hウォッチ','アルソー','プロフィール'];
const H_TERMS_A=['hermes watch near mint','hermes watch exc+5','hermes kelly watch','hermes clipper watch','hermes watch n mint'];
const H_TERMS_B=['hermes kelly watch','hermes clipper','hermes watch near mint','hermes h watch','hermes arceau'];
function hGroup(t){
  if(!/\bherm[eè]s\b/i.test(t))return '';
  if(/kelly/i.test(t))return /cadena|padlock/i.test(t)?H_GROUPS[0]:H_GROUPS[1];
  if(/clipper/i.test(t)){
    const match=t.match(/\b(\d{2})(\.\d)?\s?mm\b/i);if(!match)return '';
    const size=Number(match[1]+(match[2]||''));
    if(size>=34)return H_GROUPS[6];if(size>=27&&size<=33)return H_GROUPS[5];if(size>26)return '';
    const gold=/gold[\s-]*(plated|tone|filled)?|\bGP\b|\bGF\b|18k|750/i.test(t);
    const silver=/silver|\bSS\b|stainless|steel/i.test(t);
    const combi=/two[\s-]?tone|combi|bicolor|gold\s*(&|and|\/)\s*silver|silver\s*(&|and|\/)\s*gold/i.test(t)||(gold&&silver);
    return combi?H_GROUPS[4]:gold?H_GROUPS[2]:H_GROUPS[3];
  }
  if(/arceau/i.test(t))return H_GROUPS[9];
  if(/\bh[\s-]?(watch|our)\b|heure\s*h|\bHH1/i.test(t))return H_GROUPS[8];
  if(/rallye?/i.test(t))return H_GROUPS[7];
  if(/profil/i.test(t))return H_GROUPS[10];
  return '';
}
const titleRe=m=>m==='HERMES'?/\bherm[eè]s\b/i:cfgOf(m).re(m);
...
async function getA(m){
  ...
  const terms=m==='HERMES'?H_TERMS_A:cfg.terms(m),maxPg=m==='HERMES'?4:cfg.pages||3,ipg=m==='HERMES'?240:60;
  for(const term of terms){
    for(let pg=1;pg<=maxPg;pg++){
      const h=await fetch(`/sch/i.html?_nkw=${encodeURIComponent(term)}&LH_Sold=1&LH_BIN=1&_ipg=${ipg}${m==='HERMES'?'&_sacat=31387':''}&_blrs=spell_auto_correct&_pgn=${pg}`,{credentials:'include'}).then(r=>r.text());
      await sleep(WAIT);
...
async function getB1(kw,m,g){
  ...
  for(let off=0;off<500;off+=50){
    ...
    const got=rows.map(r=>{const c=[...r.querySelectorAll('td')].map(td=>td.innerText.replace(/\s+/g,' ').trim());const title=(c[0]||'').replace(/^, preview full size image /,'');
      ...
      const o={title,p,s,q:c[4]||'',d:c[7]||'',k:cls(title),why:rowReason(m,title)};if(g)o.g=gkey(title);return o;});
...
async function getB(m){
  const cfg=cfgOf(m);
  if(!isGucci(m)){const terms=m==='HERMES'?H_TERMS_B:cfg.terms(m);const seen={},rows=[],errs=[];
...
function calc(m,Arows,Brows,berr){
  ...
  notes.push('その他件数はB（US1年）。自動収集v3.2');
...
function calcG(m,Arows,Brows,berr){
  ...
  for(const [name,f] of groups){if(!hasData(f))continue;
    ...
    out.push(res);}
  if(!out.length)out.push({型番:m,区分:'（該当なし）',exc5:{上限:null,ラベル:'なし'},nm:{上限:null,ラベル:'なし'},その他件数:{},備考:`EXC+5・Near MINTの落札が見つからない。ケース色不明${unk}件。自動収集v3.2`});
  return out;
}
...
function modelsFromGallery(obj){const lines=Array.isArray(obj)?obj:obj.lines||[],set=[],skip=[],br={},photo={},hermesGroups=[];
  ...
  return{set,skip,br,photo,hermesGroups};}
...
<h3>相場収集ツール <span class="m">v3.2</span><button class="s" id="sb-x" style="float:right">閉じる</button></h3>
...
async function run(){
  if(running)return;
  if(!S){let list=[],skip=[],br={},photo={},hermesGroups=[];const t=$('sb-t').value.trim(),file=$('sb-f').files[0];
    if(file){const g=modelsFromGallery(JSON.parse(await file.text()));list=g.set;skip=g.skip;br=g.br;photo=g.photo;hermesGroups=g.hermesGroups;}
    ...
    const d=new Date();S={date:`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`,models:list,skip,br,photo,hermesGroups,res:{}};save(S);}
  ...
  for(const m of S.models){if(stop)break;if(S.res[m]||(S.err&&S.err[m]))continue;
    ...
    try{const A=await getA(m);const B=await getB(m);
      let r=m==='HERMES'?calcH(A,B.rows,B.err):isGucci(m)?calcG(m,A,B.rows,B.err):calc(m,A,B.rows,B.err);
