...
rr.forEach(r=>{assert.equal(r.型番,'RADO');assert.equal(r.nm.a件数,1);assert.ok(r.備考.includes('B読めず'));assert.ok(r.備考.endsWith('自動収集v3.4'));});
...
const normalize=x=>JSON.stringify(x).replaceAll('自動収集v3.4','自動収集v3.3');
...
const unchanged=src=>src.slice(src.indexOf('function quart('),src.indexOf('function calcH(')).replaceAll('自動収集v3.4','自動収集v3.3');
...
const html=read('相場収集ツール_登録_v3.4.html');
...
async function startup(){
  for(const fileInput of [true,false]){
    ...
    assert.ok(box.innerHTML.includes('<span class="m">v3.4</span>'));
  ...
  console.log('PASS: run(), gallery/manual group selection, saved state, photoCheck, v3.4 panel and output JSON');
}
Promise.all([collectors(),startup()]).catch(e=>{console.error(e);process.exitCode=1});
