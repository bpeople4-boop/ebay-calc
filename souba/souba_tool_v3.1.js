...
const KEY='soubaTool_v1',RATE=158,COEF=106,FIXED=5000,RATIO=0.897,WAIT=2500;
...
/* ---------- 仕分け ---------- */
const EXCL=/\b(links?|bracelet links|bezel|crown|strap|band|parts|clasp|end link|click ring|movement|watch case|hands?|spares|repairs|for service|refurbish|not running|junk|dial only|case only|glass|crystal|buckle)\b/i;
...
const BRANDS=[
 ...
 {key:'RADO',brand:'RADO',word:/\brado\b|ラドー/i,
  ...
  terms:m=>['rado '+m],re:m=>looseRe(m),pages:3},
 {key:'ORIS',brand:'オリス',word:/\boris\b|オリス/i,
  pat:/^(\d{3}-)?\d{4}[A-Z]?$/i,
  terms:m=>{const t=m.replace(/^\d{3}-/,'');return t===m?['oris '+m]:['oris '+t,'oris '+m]},
  re:m=>{const t=m.replace(/^\d{3}-/,'');return t===m?looseRe(m):anyRe([looseRe(m),looseRe(t)])},pages:3},
 ...
 {key:'BVLGARI',brand:'ブルガリ',word:/bvlgari|bulgari|ブルガリ/i,
  ...
  terms:m=>['bulgari '+m],re:m=>looseRe(m),pages:3},
 {key:'DIOR',brand:'ディオール',word:/dior|ディオール/i,
  pat:/^D\d{2,3}(-\d{2,3})?$/i,
  terms:m=>['dior '+m],re:m=>looseRe(m),pages:3},
...
const cfgOf=m=>/^GUCCI /.test(m)?byKey('GUCCI'):(S&&S.br&&S.br[m]?byKey(S.br[m]):BRANDS.find(b=>b.key!=='GUCCI'&&b.pat.test(m))||byKey('TAG'));
...
function gRe(m){const g=gModel(m),x=g.match(/^(\d{3,4})(?:\.(\d)\.)?([LM])$/);if(!x)return/$^/;
  const sub=x[2]?'[.\\s-]?'+x[2]:'',other=x[3]==='L'?'M':'L',suf=`\\s?[.\\-]?\\s?${x[3]}`;
  const tail=MERGE_BARE[g]?`(?:${suf})?`:suf;
  return new RegExp(`(?:^|[^\\d.])${x[1]}${sub}${tail}(?!\\d|\\.\\d|\\s?[.\\-]?\\s?${other}\\b)`,'i');}
...
async function getA(m){
  ...
  const terms=cfg.terms(isGucci(m)?m:m),maxPg=cfg.pages||3,re=cfg.re(m);
  for(const term of terms){
    for(let pg=1;pg<=maxPg;pg++){
      const h=await fetch(`/sch/i.html?_nkw=${encodeURIComponent(term)}&LH_Sold=1&LH_BIN=1&_ipg=60&_blrs=spell_auto_correct&_pgn=${pg}`,{credentials:'include'}).then(r=>r.text());
      await sleep(WAIT);
      const d=new DOMParser().parseFromString(h,'text/html');
      const head=num((d.body.textContent.match(/([\d,]+)\+?\s*results?\s+for/)||[,'0'])[1]);
      const cards=[...d.querySelectorAll('li.s-card, li.s-item')].filter(e=>/Sold\s/.test(e.textContent)).slice(0,Math.max(0,head-(pg-1)*60));
      let old=0;
      for(const e of cards){
        ...
        const usd=/^\$[\d,.]+$/.test(pr)?num(pr.slice(1)):null;
        ...
        let why='';if(!re.test(title))why='型番違い';else if(!date||date<cut)why='90日外';else if(bo)why='Best offer';else if(usd===null)why='ドル以外';else if(ship===null)why='送料不明';
...
async function getB1(kw,re,g){
  ...
  for(let off=0;off<500;off+=50){
    ...
    document.body.appendChild(f);let rows=[],ok=false;
    for(let i=0;i<25;i++){await sleep(1000);let doc;try{doc=f.contentDocument}catch(e){break}
    ...
    const got=rows.map(r=>{const c=[...r.querySelectorAll('td')].map(td=>td.innerText.replace(/\s+/g,' ').trim());const title=(c[0]||'').replace(/^, preview full size image /,'');
      const p=num((c[2]||'').match(/\$([\d,.]+)/)?.[1]||0),s=/Free/.test((c[3]||'').split('%')[0])?0:num((c[3]||'').match(/\$([\d,.]+)/)?.[1]||0);
      const o={title,p,s,q:c[4]||'',d:c[7]||'',k:cls(title),why:re.test(title)?'':'型番違い'};if(g)o.g=gkey(title);return o;});
    f.remove();out.push(...got);
    if(!ok)return {rows:out,err:'B読めず（'+kw+'・'+off+'件目から）'};
...
async function getB(m){
  const cfg=cfgOf(m);
  if(!isGucci(m)){const terms=cfg.terms(m);const seen={},rows=[],errs=[];
    for(const kw of terms){const b=await getB1(kw,cfg.re(m),false);if(b.err)errs.push(b.err);
      b.rows.forEach(r=>{const k=r.title+'|'+r.d+'|'+r.p;if(!seen[k]){seen[k]=1;rows.push(r);}});}
    return {rows,err:errs.join('／')};}
  const seen={},rows=[],errs=[];
  for(const kw of gTerms(m)){const b=await getB1(kw,gRe(m),true);if(b.err)errs.push(b.err);
    b.rows.forEach(r=>{const k=r.title+'|'+r.d+'|'+r.p;if(!seen[k]){seen[k]=1;rows.push(r);}});}
  return {rows,err:errs.join('／')};
}
...
function calc(m,Arows,Brows,berr){
  ...
  /* v3.1：上限の逆転が1,000円以内なら Near MINT を EXC+5 と同額にそろえる */
  if(e.res.上限!=null&&n.res.上限!=null&&e.res.上限>n.res.上限){const diff=e.res.上限-n.res.上限;
    if(diff<=1000){n.res.上限=e.res.上限;notes.push(`逆転のため同額にそろえた（EXC+5がNear MINTを${diff.toLocaleString()}円上回ったため）`);}
    else notes.push(`要目視：EXC+5がNear MINTを${diff.toLocaleString()}円上回っている`);}
  ...
  notes.push('その他件数はB（US1年）。自動収集v3.1');
  return{型番:m,exc5:e.res,nm:n.res,その他件数:{mint:cB.MINT||0,exc4:cB.EXC4||0,excellentpp:cB.EXCPP||0,無表記:cB.NONE||0},備考:notes.join('。')};
}
...
function calcG(m,Arows,Brows,berr){
  ...
  for(const [name,f] of groups){if(!hasData(f))continue;
    ...
    out.push(res);}
  if(!out.length)out.push({型番:m,区分:'（該当なし）',exc5:{上限:null,ラベル:'なし'},nm:{上限:null,ラベル:'なし'},その他件数:{},備考:`EXC+5・Near MINTの落札が見つからない。ケース色不明${unk}件。自動収集v3.1`});
  return out;
}
...
function modelsFromGallery(obj){const lines=Array.isArray(obj)?obj:obj.lines||[],set=[],skip=[],br={},photo={};
  lines.forEach(l=>{const cfg=byBrand(l.brand);
    (l.vars||[]).forEach(v=>{let q='';if(v.ebay){try{q=new URL(v.ebay).searchParams.get('_nkw')||''}catch(e){}}
      let m='';
...
<h3>相場収集ツール <span class="m">v3.1</span><button class="s" id="sb-x" style="float:right">閉じる</button></h3>
<div class="m">ギャラリーの書き出しJSONを選ぶか、型番を並べて入力（改行・カンマ区切り）。ブランド名を頭に付けると確実（例：gucci 1500L／rado 129.0266.3／oris 7400B／fendi 640L）。Ship To=UK を確認してから開始。</div>
...
async function run(){
  if(running)return;
  if(!S){let list=[],skip=[],br={},photo={};const t=$('sb-t').value.trim(),file=$('sb-f').files[0];
    if(file){const g=modelsFromGallery(JSON.parse(await file.text()));list=g.set;skip=g.skip;br=g.br;photo=g.photo;}
    ...
    const d=new Date();S={date:`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`,models:list,skip,br,photo,res:{}};save(S);}
  ...
  for(const m of S.models){if(stop)break;if(S.res[m]||(S.err&&S.err[m]))continue;
    ...
    try{const A=await getA(m);const B=await getB(m);
      let r=isGucci(m)?calcG(m,A,B.rows,B.err):calc(m,A,B.rows,B.err);
