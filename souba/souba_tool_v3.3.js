...
function calc(m,Arows,Brows,berr){
  ...
  notes.push('その他件数はB（US1年）。自動収集v3.3');
...
function calcG(m,Arows,Brows,berr){
  ...
  for(const [name,f] of groups){if(!hasData(f))continue;
    ...
    out.push(res);}
  if(!out.length)out.push({型番:m,区分:'（該当なし）',exc5:{上限:null,ラベル:'なし'},nm:{上限:null,ラベル:'なし'},その他件数:{},備考:`EXC+5・Near MINTの落札が見つからない。ケース色不明${unk}件。自動収集v3.3`});
  return out;
}
...
<h3>相場収集ツール <span class="m">v3.3</span><button class="s" id="sb-x" style="float:right">閉じる</button></h3>
